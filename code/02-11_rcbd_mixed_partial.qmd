...
(1|rep) this is you add the random effect into a mixed model. (1|location/rep) this would be if your running a multi-location analysis, the bigger comes first, this a nesting, there's a hierarchy which they fall under, it breaks down into (1|location) + (1|location:rep), rep is associated with location, is location + the interaction of location with rep. this is not the same as (1|rep) + (1|location) this is saying there are independent random effects. (1|year/location/rep) rep is nested in location, which is nested in year ot (1|location|year|rep) depending on your preference. (1|year:location/rep)  year interaction with location, which is nested in rep. 
...
```{r randeff_rep}
randeff_rep <- ranef(rcbd_mix_mod)[[1]] #without the [[1]] is not a data frame
