...
#nrate_kgha*krate_kgha = nrate_kgha + krate_kgha + nrate_kgha:krate_kgha
#adding rep on lm, is what differentiate from crd from rcbd
# Summary
summary(rcbd_mod)
```
...
- Residual independence (no pattern)  #you check on residuals, not raw data.
...
- you use -3,3 because of the standard deviation under the curve, 3x standard deviation is were 99% of the data is. 
...
#originally is nrate...:krate..., but for the final plot it was change to krate..|nrate...
#expected marginal means = emmeans
rcbd_means_all

#k inside n
rcbd_means_kn <-emmeans(rcbd_mod,
                        ~krate_kgha|nrate_kgha)
rcbd_means_kn

#n inside k
rcbd_means_nk <- emmeans(rcbd_mod,
                         ~nrate_kgha|krate_kgha)
rcbd_means_nk
...
rcbd_cld_all <- cld(rcbd_means_all, 
                   reversed=T, 
                   adjust="none",
               Letters=letters) #you can add a different alpha, just add alpha = 0.1 after letters but dont forget to add a , before alpha so the code can run. 
#the more comparison you make, the more false positive you have, that means interaction you might found not be true interactions. 
#fisher lsd means they did not use an adjustment, they decided how to break their comparison based on the anova table.
#adjustment is a way to say i have the certainty about a level or trt. 
rcbd_cld_all
...
rcbd_cld_selected <- rcbd_cld_all %>%
  ...
  mutate(trtname = paste0(nrate_kgha, "+", krate_kgha)) #this mutate, you can add it in the previous mutate, you dont have to add another mutate. 
...
```{r rcbd final plot}
#going to add two datas, the raw and the other one with the comparison
ggplot() +
  geom_boxplot(data=rcbd_dfw,
               aes(x = krate_kgha,
                   y = yield_kgha,
                   fill = trtname) #you can also use fill, but fill will colored the inside of the plot
               ) +
